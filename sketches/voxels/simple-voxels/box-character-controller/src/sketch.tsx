...
import { CameraBuildTool } from '../../lib/camera-build-tool'
import { VoxelChunkMeshes, Voxels, useVoxels } from '../../lib/react'
import { useSimpleLevel } from '../../lib/simple-level'
...
const Player = ({ position: initialPosition, width, height }: PlayerProps) => {
    ...
    const intersectsVoxel = (worldPosition: THREE.Vector3Like) => {
        const position = _intersectsVoxelPosition.copy(worldPosition).floor()
        return voxels.world.getSolid(position.x, position.y, position.z)
    }
