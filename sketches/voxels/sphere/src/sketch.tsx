...
const Example = () => {
	...
	const [diamondTexture, stoneTexture] = suspend(async () => {
		return await Promise.all(
			[diamondTextureUrl, stoneTextureUrl].map(loadImage),
		);
			...
			for (let y = -size; y < size; y++) {
				for (let z = -size; z < size; z++) {
					if (x ** 2 + y ** 2 + z ** 2 < radius ** 2) {
						const blockType =
							Math.random() > 0.5 ? stoneBlock.index : diamondBlock.index;
	...
	}, [diamondTexture, stoneTexture, scene]);
...
export function Sketch() {
	return (
		<WebGPUCanvas camera={{ position: [20, 10, 20] }} gl={{ antialias: true }}>
			<Example />
