...
import { type AttributeNode, InterpolationSamplingMode, InterpolationSamplingType, MeshStandardNodeMaterial } from 'three/webgpu';

export class ChunkMaterial extends MeshStandardNodeMaterial {
    updateTexture(atlasTexture: CanvasTexture) {
        this.colorNode = Fn(() => {
            const ao = attribute('ao', 'float') as AttributeNode<'float'>;

            const atlasUv = uv()
                .toVarying()
                .setInterpolation(InterpolationSamplingType.PERSPECTIVE, InterpolationSamplingMode.CENTROID);
