...
import type * as BlockRegistry from './block-registry';
import type * as TextureAtlas from './texture-atlas';
import { CHUNK_SIZE, type Chunk, type World } from './world';
...
const getType = (
    ...
    return world.getBlock(
        x + chunk.worldPositionOffset.x,
        y + chunk.worldPositionOffset.y,
        z + chunk.worldPositionOffset.z,
    );
};
...
export const mesh = (
        ...
        for (let z = -1; z < CHUNK_SIZE; z++) {
            for (let y = -1; y < CHUNK_SIZE; y++) {
                ...
                for (let dir = 0; dir < 3; dir++) {
                    ...
                    const marchNeighbourBlockSolid =
                        marchNeighbourBlockType !== 0;

                    if (marchBlockSolid === marchNeighbourBlockSolid) continue;

                    const side = marchBlockSolid ? 0 : 1;
                    const faceBlockType = side
                        ? marchNeighbourBlockType
                        : marchBlockType;

                    const faceBlockTypeDetails =
                        blockRegistry.blockIndexToBlock.get(faceBlockType);
                    ...
                    _opaqueBuffer.positions[positionsIndex++] =
                        marchNeighbourX + ux;
                    _opaqueBuffer.positions[positionsIndex++] =
                        marchNeighbourY + uy;
                    _opaqueBuffer.positions[positionsIndex++] =
                    ...
                    _opaqueBuffer.positions[positionsIndex++] =
                        marchNeighbourX + ux + vx;
                    _opaqueBuffer.positions[positionsIndex++] =
                        marchNeighbourY + uy + vy;
                    _opaqueBuffer.positions[positionsIndex++] =
                        marchNeighbourZ + uz + vz;
                    ...
                    const cubeLayout = textureAtlasLayout.blockCubeLayouts.get(
                        faceBlockTypeDetails.cube,
                    );
                    ...
                    for (let q = -1; q < 2; q++) {
                        for (let p = -1; p < 2; p++) {
                            const aoNeighbourX =
                                _blockPosition.x + dx + ux * p + vx * q;
                            const aoNeighbourY =
                                _blockPosition.y + dy + uy * p + vy * q;
                            const aoNeighbourZ =
                                _blockPosition.z + dz + uz * p + vz * q;
                    ...
                    const ao00 = vertexAmbientOcclusion(
                        aoGrid[3],
                        aoGrid[1],
                        aoGrid[0],
                    );
                    const ao01 = vertexAmbientOcclusion(
                        aoGrid[1],
                        aoGrid[5],
                        aoGrid[2],
                    );
                    const ao10 = vertexAmbientOcclusion(
                        aoGrid[5],
                        aoGrid[7],
                        aoGrid[8],
                    );
                    const ao11 = vertexAmbientOcclusion(
                        aoGrid[3],
                        aoGrid[7],
                        aoGrid[6],
                    );
    ...
    return {
        positions: new Float32Array(
            _opaqueBuffer.positions.slice(0, positionsIndex),
        ),
