...
import { Instructions, Controls, WebGPUCanvas } from '@sketches/common';
...
import { type Chain, JointConstraintType, type Vec3, bone, fabrikFixedIterations } from './fabrik';
...
const crawlerQuery = world.query((e) => e.is('crawler').and.has('rigidBody', 'three'));
const controlTargetCrawlerQuery = world.query((e) => e.is('isControlTarget').and.has('crawler').and.has('rigidBody', 'three'));
...
const updateCrawlerMovement = (crawler: CrawlerState, rigidBody: RapierRigidBody, dt: number) => {
...
const updateCrawlerPosition = (crawler: CrawlerState, rigidBody: RapierRigidBody) => {
...
const updateCrawlerSuspension = (crawler: CrawlerState, rigidBody: RapierRigidBody, world: Rapier.World, dt: number) => {
    if (crawler.state.jumping && (crawler.state.lastJumpTime + 300 > performance.now() || rigidBody.linvel().y > 0)) {
        return;
    }
    ...
    const rayColliderIntersection = world.castRayAndGetNormal(ray, rayLength, false, undefined, undefined, undefined, rigidBody);

    let grounded = false;

    if (rayColliderIntersection?.timeOfImpact !== undefined) {
        const distance = _rayDistance.copy(_rayDirection).multiplyScalar(rayColliderIntersection?.timeOfImpact ?? 1);
...
const remapClamp = (value: number, inMin: number, inMax: number, outMin: number, outMax: number) => {
...
const updateCrawlerFootPlacement = (
    ...
    if (crawler.state.grounded) {
        for (const leg of crawler.def.legs) {
            ...
            const ray = new Rapier.Ray(legState.footPlacementRayOrigin, _rayDirection);
...
const updateCrawlerStepping = (crawler: CrawlerState, rigidBody: RapierRigidBody, dt: number) => {
    for (const leg of crawler.def.legs) {
        const legState = crawler.state.legs[leg.id];
...
const initFootPlacementHelper = (scene: Scene) => {
    const rayOriginHelper = new Mesh(new SphereGeometry(0.05), new MeshBasicMaterial({ color: 'red', wireframe: true }));

    const targetPositionHelper = new Mesh(new SphereGeometry(0.05), new MeshBasicMaterial({ color: 'green', wireframe: true }));

    const currentPositionHelper = new Mesh(new SphereGeometry(0.06), new MeshBasicMaterial({ color: 'blue', wireframe: true }));
    ...
    const materials: Material[] = [rayOriginHelper.material, targetPositionHelper.material, currentPositionHelper.material];
...
const initChainHelper = (chain: Chain, object: Object3D) => {
    ...
    for (let i = 0; i < chain.bones.length; i++) {
        ...
        const jointMesh = new Mesh(jointGeometry, i === 0 ? baseMaterial : boneMaterial);
...
const updateChainHelper = (leg: LegDef, chain: Chain, chainHelper: ChainHelper) => {
    for (let i = 0; i < chain.bones.length; i++) {
        const bone = chain.bones[i];
...
const updateCrawlerDebugVisuals = (crawler: CrawlerState, debug: boolean, object: Object3D, scene: Scene) => {
    for (const leg of crawler.def.legs) {
        const legState = crawler.state.legs[leg.id];
...
const initLegVisuals = (crawlerDef: CrawlerDef, chain: Chain, object: Object3D) => {
    // cylinders for each bone
    const boneMeshes: Mesh[] = [];
...
const updateLegVisuals = (leg: LegDef, chain: Chain, legVisuals: LegVisuals) => {
    for (let i = 0; i < chain.bones.length; i++) {
        const bone = chain.bones[i];
...
const CrawlerGooglyEye = ({
    ...
    useFrame((_, delta) => {
        ...
        _addVelocity.y =
            Math.abs(_movementVelocity.y) > Math.abs(_movementVelocity.z) ? _movementVelocity.y : -_movementVelocity.z;
        ...
        if (distance > maxDistance) {
            ...
            localPosition.current.set(Math.cos(angle) * maxDistance, Math.sin(angle) * maxDistance, 0);
        }
    ...
    return (
        <group {...groupProps} ref={eyeRef}>
            ...
            <mesh scale={[1, 1, 0.4]}>
                ...
                <meshStandardMaterial transparent opacity={0.1} color="#fff" roughness={0.2} metalness={0.5} />
            </mesh>
        </group>
    );
};
...
const Crawler = ({ ref, def, debug = false, isControlTarget = false, ...rigidBodyProps }: CrawlerProps) => {
    ...
    return (
        <RigidBody {...rigidBodyProps} type="dynamic" shape={undefined} colliders={false} lockRotations ref={rigidBodyRef}>
            <group ref={groupRef}>
                ...
                <CrawlerGooglyEye position={[-0.25, 0.4, 0.5]} rotation={[-0.6, 0, 0]} eyeRadius={0.2} irisRadius={0.075} />
                <CrawlerGooglyEye position={[0.25, 0.4, 0.5]} rotation={[-0.6, 0, 0]} eyeRadius={0.2} irisRadius={0.075} />
            </group>
        </RigidBody>
    );
...
const heightFieldGeometry = new PlaneGeometry(heightFieldWidth, heightFieldDepth, heightFieldWidth - 1, heightFieldDepth - 1);
...
const Heightfield = () => {
    ...
    useEffect(() => {
        const heightfieldDesc = Rapier.ColliderDesc.heightfield(heightFieldWidth - 1, heightFieldWidth - 1, heightField, {
            x: heightFieldWidth,
            y: 1,
            z: heightFieldDepth,
        });
...
const Environment = () => (
    <>
        ...
        {[...Array(4)].map((_, i) => (
            <RigidBody key={String(i)} type="fixed" shape="cuboid" position={[i * -2 - 15, 1 + i * 0.5, -5]}>
                <mesh castShadow receiveShadow>
        ...
        {/* stairs */}
        {[...Array(10)].map((_, i) => (
            <RigidBody key={String(i)} type="fixed" shape="cuboid" position={[i * 1.5 - 9, -0.5 + i * 0.2, 2]}>
                <mesh castShadow receiveShadow>
...
const App = () => {
    ...
    useFrame(({ camera, scene }, frameDt) => {
        ...
        for (const entity of crawlerQuery) {
            ...
            updateCrawlerSuspension(entity.crawler, entity.rigidBody, rapierRef.current.world, dt);
        }
        ...
        for (const entity of crawlerQuery) {
            ...
            updateCrawlerFootPlacement(entity.crawler, entity.three, entity.rigidBody, rapierRef.current.world);
        ...
        if (!debug && controlTargetCrawler) {
            cameraTarget.current.lerp(controlTargetCrawler.crawler.state.position, dt * 5);
            camera.position.copy(cameraTarget.current).add(_cameraOffset.set(0, 5, 15));
    ...
    return (
        <>
            <Physics debug={debug} gravity={[0, -20, 0]} timeStep="vary" paused>
                ...
                <Crawler position={[0, 10, 2]} def={crawlerDef} debug={debug} isControlTarget />
...
export function Sketch() {
    return (
        <>
            ...
            <Instructions>* wasd to move, space to jump, shift to sprint, c to crouch</Instructions>
