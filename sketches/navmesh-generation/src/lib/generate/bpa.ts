...
// referenced: https://github.com/bernhardmgruber/bpa
...
const createGrid = (
    ...
    const cells: Cell[] = new Array(dims[0] * dims[1] * dims[2])
        .fill(null)
        .map(() => []);
    ...
    for (const p of points) {
        ...
        const cellIdx =
            index[2] * dims[0] * dims[1] + index[1] * dims[0] + index[0];
        cells[cellIdx].push({
...
const getSphericalNeighborhood = (
    grid: Grid,
    point: Vec3,
    ignore: Vec3[],
): MeshPoint[] => {
    const result: MeshPoint[] = [];
    const centerIndex = getCellIndex(
        point,
        grid.lower,
        grid.dims,
        grid.cellSize,
    ...
    for (let xOff = -1; xOff <= 1; xOff++) {
        for (let yOff = -1; yOff <= 1; yOff++) {
            for (let zOff = -1; zOff <= 1; zOff++) {
                ...
                const cellIdx =
                    index[2] * grid.dims[0] * grid.dims[1] +
                    index[1] * grid.dims[0] +
                    index[0];
                ...
                for (const p of cell) {
                    ...
                    if (distSq < grid.cellSize * grid.cellSize) {
                        const isIgnored = ignore.some((ignorePos) =>
                            vec3.exactEquals(p.pos, ignorePos),
                        );
                        if (!isIgnored) {
                            result.push(p);
                        }
...
const computeBallCenter = (face: MeshFace, radius: number): Vec3 | null => {
    ...
    vec3.scale(
        toCircumCircleCenter,
        toCircumCircleCenter,
        1 / (2 * abXacLengthSq),
    );

    const circumCircleCenter = vec3.add(
        vec3.create(),
        face[0].pos,
        toCircumCircleCenter,
    );
    ...
    const heightSquared =
        radius * radius - vec3.squaredLength(toCircumCircleCenter);
    if (heightSquared < 0) return null;

    const faceNormal = getMeshFaceNormal(face);
    const ballCenter = vec3.scaleAndAdd(
        vec3.create(),
        circumCircleCenter,
        faceNormal,
...
const ballIsEmpty = (
    ...
    return !points.some(
        (p) => vec3.squaredDistance(p.pos, ballCenter) < radiusSq,
    );
};
...
const findSeedTriangle = (grid: Grid, radius: number): SeedResult | null => {
    for (const cell of grid.cells) {
        ...
        for (const p1 of unusedPoints) {
            const neighborhood = getSphericalNeighborhood(grid, p1.pos, [
                p1.pos,
            ...
            for (const p2 of neighborhood) {
                for (const p3 of neighborhood) {
                    ...
                    if (
                        ballCenter &&
                        ballIsEmpty(ballCenter, neighborhood, radius)
                    ) {
                        p1.used = true;
                        p2.used = true;
                        p3.used = true;
...
const ballPivot = (
    ...
    const m = vec3.scale(
        vec3.create(),
        vec3.add(vec3.create(), e.a.pos, e.b.pos),
        0.5,
    );
    const oldCenterVec = vec3.normalize(
        vec3.create(),
        vec3.sub(vec3.create(), e.center, m),
    );
    const neighborhood = getSphericalNeighborhood(grid, m, [
        e.a.pos,
        e.b.pos,
        e.opposite.pos,
    ]);
    ...
    for (const p of neighborhood) {
        ...
        const newCenterVec = vec3.normalize(
            vec3.create(),
            vec3.sub(vec3.create(), c, m),
        );
        ...
        for (const ee of p.edges) {
            ...
            if (
                ee.status === EdgeStatus.inner &&
                (otherPoint === e.a || otherPoint === e.b)
            ) {
                hasInnerEdge = true;
                break;
            }
        ...
        let angle = Math.acos(
            Math.max(-1, Math.min(1, vec3.dot(oldCenterVec, newCenterVec))),
        );
...
const outputTriangle = (
    ...
    const dotProduct = vec3.dot(faceNormal, yUp);
    const angleRadians = Math.acos(Math.max(-1, Math.min(1, dotProduct)));
    const angleDegrees = (angleRadians * 180) / Math.PI;

    if (angleDegrees <= walkableSlopeAngle) {
        triangles.push({
...
const runBPAFromSeed = (
    ...
    outputTriangle(seed, triangles, walkableSlopeAngle);
