...
const outputTriangle = (face: MeshFace, triangles: Triangle[], walkableSlopeAngle: number): void => {
    const faceNormal = getMeshFaceNormal(face);
...
const runBPAFromSeed = (
    ...
    while (true) {
        ...
        if (o_k && (notUsed(o_k.p) || onFront(o_k.p))) {
            outputTriangle([e_ij.a, o_k.p, e_ij.b], triangles, walkableSlopeAngle);
...
export const pointSetToWalkableTriangleMeshBPA = (
    ...
    while (true) {
        ...
        runBPAFromSeed(seedResult, grid, radius, walkableSlopeAngle, triangles, edges);
    }
