...
export const Agent = ({ position }: AgentProps) => {
    ...
    useInterval(() => {
        ...
        const { point: playerPosition } = navMeshQuery.findClosestPoint(player.rigidBody.translation(), {
    ...
    useBeforePhysicsStep(() => {
        ...
        if (pathIndex.current === path.length - 1) {
            if (horizontalDistance(next, ref.current.translation()) < distanceThreshold) {
                return
            }
        }
    ...
    return (
        <>
            ...
            {pathDebugLine && path.length > 0 && <Line points={path} color="blue" lineWidth={2} position={[0, 0.2, 0]} />}
        </>
    )
}
