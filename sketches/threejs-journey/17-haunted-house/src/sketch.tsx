    ...
    useLayoutEffect(() => {
        if (wallsBoxGeometryRef.current) {
            wallsBoxGeometryRef.current.setAttribute(
                'uv2',
                new BufferAttribute((wallsBoxGeometryRef.current.attributes.uv as BufferAttribute).array, 2),
            )
        }
        ...
        if (doorPlaneGeometryRef.current) {
            doorPlaneGeometryRef.current.setAttribute(
                'uv2',
                new BufferAttribute((doorPlaneGeometryRef.current.attributes.uv as BufferAttribute).array, 2),
            )
        }
    }, [])
    ...
    return (
        <>
            <group {...props}>
                ...
                {/* Roof */}
                <mesh castShadow receiveShadow position={[0, 2.5 + 0.5, 0]} rotation={[0, Math.PI / 4, 0]}>
                ...
                <mesh position={[0, 1, 2.001]}>
                    <planeGeometry ref={doorPlaneGeometryRef} args={[2.2, 2.2, 100, 100]} />
                    <meshStandardMaterial
                        transparent
                        {...doorTextureProps}
                        aoMapIntensity={2}
    ...
    useLayoutEffect(() => {
        if (planeGeometryRef.current) {
            planeGeometryRef.current.setAttribute(
                'uv2',
                new BufferAttribute((planeGeometryRef.current.attributes.uv as BufferAttribute).array, 2),
            )
        }
    }, [])
    ...
    const graves = useMemo(() => {
        return Array.from({ length: 50 })
            ...
            .map(() => {
                ...
                const rotation = [0, (Math.random() - 0.5) * 0.4, (Math.random() - 0.5) * 0.4] as [number, number, number]
    ...
    return (
        <>
            ...
            {graves.map((grave, idx) => (
                <Grave key={idx} position={grave.position} rotation={grave.rotation} />
            ))}
        </>
    )
}
...
export function Sketch() {
    return (
        <Canvas camera={{ position: [6, 6, 12], fov: 50 }} shadows={{ type: THREE.PCFSoftShadowMap }}>
            <App />
            <OrbitControls />
        </Canvas>
    )
