        ...
        >
            ...
            <Box padding={padding} centerAnchor>
                <Float>
                    <mesh>
                        <boxGeometry args={[1, 1, 1]} />
                        <meshPhongMaterial shininess={10} specular={0x0000ff} color={0xff0000} />
                    </mesh>
                </Float>
            </Box>
            <Box padding={padding} centerAnchor>
                <Float>
                    <mesh>
                        <boxGeometry args={[1, 1, 1]} />
                        <meshStandardMaterial metalness={0.45} roughness={0.65} color={0xff0000} />
                    </mesh>
                </Float>
            </Box>
        </Flex>
    ...
    useLayoutEffect(() => {
        if (planeGeometryRef.current) {
            planeGeometryRef.current.setAttribute(
                'uv2',
                new THREE.BufferAttribute((planeGeometryRef.current.attributes.uv as BufferAttribute).array, 2),
            )
        }
    }, [])
        ...
        <>
            <Float floatIntensity={5} position={props.position}>
                <mesh>
                    ...
                    <meshStandardMaterial {...doorTextureProps} transparent aoMapIntensity={1.2} displacementScale={0.2} />
                </mesh>
            </Float>
        </>
    )
