...
export const Network = ({ children, ...groupProps }: NetworkProps) => {
    ...
    useEffect(() => {
        ...
        simulation
            .force<d3.ForceLink<NetworkNode, DedupedLink>>('link')!
            .links(validDedupedLinks)
    ...
    return (
        <networkContext.Provider
            value={{ addNode, removeNode, addLink, removeLink }}
        >
...
export const Node = ({ children, id, fixedPosition }: NodeProps) => {
    ...
    useEffect(() => {
        const [fx, fy] = fixedPosition
            ? [
                  fixedPosition[0] * viewport.factor,
                  fixedPosition[1] * viewport.factor,
              ]
            : [undefined, undefined]
